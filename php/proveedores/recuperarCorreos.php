<?php 
    $id = $_GET['idProveedor']; //Recupera el ID del cliente por metodo GET

    $correo = ''; //Inicializacion de variables
    $iterador = 1;

    $sql = "SELECT Correo FROM Correos_Proveedores WHERE Id_Proveedor = '" . $id . "';"; //Consulta de todos los correos registrados bajo la identidad de ese cliente
    ...
    if($registros == 0){ //Si no hay correos registrados para ese cliente imprime solo el primer campo de correos txtEmail1 vacio (como estaria por defecto)
        ...
        "<div class='col-xs-12 col-sm-12 col-md-4 col-lg-4 col-xl-4'>
            <div class='form-group mt-2 mb-2'>
                ...
                <input style='float: left;' type='email' class='form-control' name='txtEmail1' id='txtEmail1' maxlength='45' placeholder='Ingrese el correo electrónico del proveedor' value=''>
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if($iterador == 1){ //Si es el primer registro crea el primer txtEmail1 por defecto pero llenado con el correo recuperado
            ...
            <div class='form-group mt-2 mb-2'>
                <label for='txtEmail1' class='form-label'>Correo Electrónico</label>
                <input style='float: left;' type='email' class='form-control' name='txtEmail1' id='txtEmail1' maxlength='45' placeholder='Ingrese el correo electrónico del proveedor' value='".$correo."'>
                <button type='button' name='add' id='btn_anadirCorreo' class='btn btn-primary mx-3 mb-1' style='background-color: #6AB759; border-color: #6AB759;'>+</button>
            </div>
        </div>";
            $iterador++;
            ...
            $iterador++;
            //Esto se hace para evitar conflictos de nombres de div y campos cuando se desee agregar mas correos (esperando que sea imposible que un proveedor tenga mas de 10 telefonos a su nombre)
        }
    }
?>